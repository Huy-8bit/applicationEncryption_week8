#include<iostream>
#include<string>
#include<cstring>
#include<iomanip>
#include<vector>

using namespace std;



// S-box
const unsigned char sbox[256] = {
    
};
unsigned char inv_sbox[256] =
{
   
};

const unsigned char rcon_table[11] = {
    0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36,
};


// Circular left shift operation on a word
void LeftShift(vector<unsigned char>& word) {
    unsigned char temp = word[0];
    for (int i = 0; i < 3; i++) {
        word[i] = word[i + 1];
    }
    word[3] = temp;
}

// Byte substitution operation on a word
void SubWord(vector<unsigned char>& word) {
    for (int i = 0; i < 4; i++) {
        unsigned char byte = word[i];
        word[i] = sbox[byte];
    }
}

// Generate the round constant for the ith round
vector<unsigned char> Rcon(int i) {
    vector<unsigned char> rcon(4);
    rcon[0] = rcon_table[i];
    rcon[1] = rcon[2] = rcon[3] = 0x00;
    return rcon;
}

// Key expansion function
vector<vector<unsigned char> > KeyExpansion(vector<unsigned char> key) {
    int key_size = key.size();
    int num_rounds = 10 + (key_size / 4 - 4);

    vector<vector<unsigned char> > round_keys(num_rounds + 1, vector<unsigned char>(16));

    // Copy the original key into the first round key
    for (int i = 0; i < key_size; i++) {
        round_keys[0][i] = key[i];
    }

    // Generate the remaining round keys
    vector<unsigned char> temp(4);
    for (int i = 1; i <= num_rounds; i++) {
        // Compute the first 4 bytes of the ith round key
        for (int j = 0; j < 4; j++) {
            temp[j] = round_keys[i - 1][j + 12];
        }
        LeftShift(temp);
        SubWord(temp);
        vector<unsigned char> rcon = Rcon(i);
        for (int j = 0; j < 4; j++) {
            temp[j] ^= rcon[j];
        }
        for (int j = 0; j < 4; j++) {
            round_keys[i][j] = round_keys[i - 1][j] ^ temp[j];
        }

        // Compute the remaining 12 bytes of the ith round key
        for (int j = 4; j < 16; j++) {
            round_keys[i][j] = round_keys[i][j - 4] ^ round_keys[i - 1][j];
        }
    }

    return round_keys;
}




void SubBytes(unsigned char state[][4]) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            state[i][j] = sbox[state[i][j]];
        }
    }
}
void InvSubBytes(unsigned char state[][4]) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            state[i][j] = inv_sbox[state[i][j]];
        }
    }
}

uint8_t gmul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    uint8_t hi_bit_set;
    for (int i = 0; i < 8; i++) {
        if (b & 1) {
            p ^= a;
        }
        hi_bit_set = (a & 0x80);
        a <<= 1;
        if (hi_bit_set) {
            a ^= 0x1b; // (0001 1011) in binary, corresponds to the irreducible polynomial x^8 + x^4 + x^3 + x + 1
        }
        b >>= 1;
    }
    return p;
}


void mixColumns(unsigned char state[4][4]) {
    unsigned char temp[4];

    for (int i = 0; i < 4; ++i) {
        temp[0] = gmul(0x02, state[i][0]) ^ gmul(0x03, state[i][1]) ^ state[i][2] ^ state[i][3];
        temp[1] = state[i][0] ^ gmul(0x02, state[i][1]) ^ gmul(0x03, state[i][2]) ^ state[i][3];
        temp[2] = state[i][0] ^ state[i][1] ^ gmul(0x02, state[i][2]) ^ gmul(0x03, state[i][3]);
        temp[3] = gmul(0x03, state[i][0]) ^ state[i][1] ^ state[i][2] ^ gmul(0x02, state[i][3]);

        state[i][0] = temp[0];
        state[i][1] = temp[1];
        state[i][2] = temp[2];
        state[i][3] = temp[3];
    }
}

void invMixColumns(unsigned char state[][4]) {
    unsigned char temp[4];
    for (int i = 0; i < 4; i++) {
        temp[0] = gmul(state[0][i], 0x0e) ^ gmul(state[1][i], 0x0b) ^ gmul(state[2][i], 0x0d) ^ gmul(state[3][i], 0x09);
        temp[1] = gmul(state[0][i], 0x09) ^ gmul(state[1][i], 0x0e) ^ gmul(state[2][i], 0x0b) ^ gmul(state[3][i], 0x0d);
        temp[2] = gmul(state[0][i], 0x0d) ^ gmul(state[1][i], 0x09) ^ gmul(state[2][i], 0x0e) ^ gmul(state[3][i], 0x0b);
        temp[3] = gmul(state[0][i], 0x0b) ^ gmul(state[1][i], 0x0d) ^ gmul(state[2][i], 0x09) ^ gmul(state[3][i], 0x0e);
        state[0][i] = temp[0];
        state[1][i] = temp[1];
        state[2][i] = temp[2];
        state[3][i] = temp[3];
    }
}


void shiftRows(unsigned char state[4][4]) {
    // Shift second row one position to the left
    unsigned char temp = state[1][0];
    state[1][0] = state[1][1];
    state[1][1] = state[1][2];
    state[1][2] = state[1][3];
    state[1][3] = temp;

    // Shift third row two positions to the left
    temp = state[2][0];
    state[2][0] = state[2][2];
    state[2][2] = temp;
    temp = state[2][1];
    state[2][1] = state[2][3];
    state[2][3] = temp;

    // Shift fourth row three positions to the left
    temp = state[3][0];
    state[3][0] = state[3][3];
    state[3][3] = state[3][2];
    state[3][2] = state[3][1];
    state[3][1] = temp;
}


void invShiftRows(unsigned char state[4][4]) {
    unsigned char temp;

    // Rotate row 1 by 3
    temp = state[1][3];
    state[1][3] = state[1][2];
    state[1][2] = state[1][1];
    state[1][1] = state[1][0];
    state[1][0] = temp;

    // Rotate row 2 by 2
    temp = state[2][0];
    state[2][0] = state[2][2];
    state[2][2] = temp;

    temp = state[2][1];
    state[2][1] = state[2][3];
    state[2][3] = temp;

    // Rotate row 3 by 1
    temp = state[3][0];
    state[3][0] = state[3][1];
    state[3][1] = state[3][2];
    state[3][2] = state[3][3];
    state[3][3] = temp;
}


void AddRoundKey(unsigned char state[][4], unsigned char round_key[][4]) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            state[i][j] ^= round_key[i][j];
        }
    }
}

void aes_encrypt_block(unsigned char state[][4], unsigned char key[][4]) {
    AddRoundKey(state, key);
    for (int i = 1; i < 10; i++) {
        SubBytes(state);
        shiftRows(state);
        mixColumns(state);
        AddRoundKey(state, key);
    }
    SubBytes(state);
    shiftRows(state);
    AddRoundKey(state, key);
}

void aes_decrypt_block(unsigned char state[][4], unsigned char key[][4]) {
    AddRoundKey(state, key);
    invShiftRows(state);
    InvSubBytes(state);
    for (int i = 1; i < 10; i++) {
        AddRoundKey(state, key);
        invMixColumns(state);
        invShiftRows(state);
        InvSubBytes(state);
    }
    AddRoundKey(state, key);
}

int main() {

    unsigned char state[4][4] = {
            {0x32, 0x88, 0x31, 0xe0},
            {0x43, 0x5a, 0x31, 0x37},
            {0xf6, 0x30, 0x98, 0x07},
            {0xa8, 0x8d, 0xa2, 0x34}
    };

    unsigned char key[4][4] = {
            {0x2b, 0x28, 0xab, 0x09},
            {0x7e, 0xae, 0xf7, 0xcf},
            {0x15, 0xd2, 0x15, 0x4f},
            {0x16, 0xa6, 0x88, 0x3c}
    };


    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            printf("%02x ", state[i][j]);
        }
        printf(" ");
    }
    cout << endl;
    aes_encrypt_block(state, key);

    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            printf("%02x ", state[i][j]);
        }
        printf(" ");
    }
    cout << endl;
    aes_decrypt_block(state, key);

    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            printf("%02x ", state[i][j]);
        }
        printf(" ");
    }
    cout << endl;

    return 0;
}